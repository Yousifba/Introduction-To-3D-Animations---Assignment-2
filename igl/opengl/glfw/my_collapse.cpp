...
IGL_INLINE bool my_collapse::collapse_edge(
	const int e,
...
{
	int e1, e2, f1, f2;
	return collapse_edge(e, p, V, F, E, EMAP, EF, EI, e1, e2, f1, f2, viewer);
}

IGL_INLINE bool my_collapse::my_collapse_e(
	Eigen::MatrixXd& V,
	Eigen::MatrixXi& F,
	Eigen::MatrixXi& E,
	Eigen::VectorXi& EMAP,
	...
	igl::opengl::glfw::Viewer* viewer)
{
	...
	return
		collapse_edge(always_try, never_care,
			V, F, E, EMAP, EF, EI, Q, Qit, C, e, e1, e2, f1, f2, viewer);
}
	...
	igl::opengl::glfw::Viewer* viewer)
{
	int e, e1, e2, f1, f2;
	return
		collapse_edge(pre_collapse, post_collapse,
			V, F, E, EMAP, EF, EI, Q, Qit, C, e, e1, e2, f1, f2, viewer);
}
	...
	igl::opengl::glfw::Viewer* viewer)
{
	...
	{
		collapsed = collapse_edge(e, C.row(e), V, F, E, EMAP, EF, EI, e1, e2, f1, f2, viewer);
	}
	else
	{
		...
		for (auto n : N)
		{
				...
				F(n, 2) != IGL_COLLAPSE_EDGE_NULL)
			{
				for (int v = 0;v < 3;v++)
				{
					...
					double cost;
					RowVectorXd place;
					viewer->quadric_error_edge(ei, V, F, E, EMAP, EF, EI, cost, place);
					// Replace in queue
	...
	if (collapsed)
	{
		//std::printf("edge %7d, cost = %.15f, new v position (%15.12f, %15.12f, %15.12f)\n", p.second, p.first, C.row(p.second).x(), C.row(p.second).y(), C.row(p.second).z());
		std::cout << "edge " << p.second << ", cost = " << p.first << ", new v position (" << C.row(p.second).x() << ", " << C.row(p.second).y() <<
			", " << C.row(p.second).z() << ")\n";
	}
	return collapsed;
}
